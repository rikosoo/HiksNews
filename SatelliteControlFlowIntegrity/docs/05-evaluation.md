...
|---|---|---|
| **Attack detection** | **6/6 (100%)** | ATK-1..6, uma execução completa cada |
...
| **Detection latency** | **26 – 1376 instruções (0,001 – 0,055 ms @ 25 MHz)** | da escrita que corrompe a pilha até a primeira aresta ilegal |
...
| Modelo de CFG (lado do monitor) | 958 KB | JSON não comprimido |
| Extração do modelo (build time) | 0,2 s | uma vez por build |
| Throughput do monitor | ~51 MB de trace/s | 4,1 GB analisados em 80 s |
...
## A lacuna de interrupções, fechada e medida

A primeira versão deste monitor aceitava entrada e retorno de exceção sem
verificar, o que era uma lacuna em relação ao SHERLOC. Ela foi fechada portando
o algoritmo *interrupt- and scheduling-aware* daquele trabalho (Apache-2.0):

- **Entrada de exceção** permanece legal — o desvio é feito pelo hardware — mas
  o endereço interrompido passa a ser **registrado** numa shadow stack.
- **Retorno de exceção** só é legal em três destinos: o código interrompido, a
  entrada de outro handler (aninhamento), ou o ponto de retomada de uma task
  estacionada por uma troca de contexto. Qualquer outro é violação.
- Os handlers passaram a ser lidos da **tabela de vetores do binário**, e não de
  uma lista de nomes conhecidos. A lista perdia handlers que a aplicação
  adiciona — e, pior, marcava a entrada legítima deles como violação.

O ATK-6 foi construído para medir o ganho. Ele transborda um buffer **dentro do
contexto de exceção** e forja o `EXC_RETURN` que o epílogo do handler joga no PC:

| Política | Violações | Onde detecta |
|---|---|---|
| `exempt` (antiga) | 1 | instrução 8.204.044 — **só o crash posterior** |
| `checked` (shadow stack) | 3 | instrução 8.201.863 — **o desvio em si** |

A diferença é de 2.181 instruções, mas o número não é o ponto: na política antiga
o sequestro em si é **invisível**, e o único alerta vem do firmware caindo depois
que `eps_kill_switch()` já executou. É um alerta post-mortem. Um atacante cujo
payload retornasse limpo não deixaria alerta nenhum.

O custo de fechar a lacuna foi **zero falso positivo adicional**: S0, S1 e S4
seguem em 0, com 3.144 entradas de exceção, 1.454 retornos, 3.641 trocas de
contexto e 3.636 retomadas de task conferidas numa única execução nominal.
...
- **Chamadas indiretas exigem CFG conservador.** O conjunto de alvos permitidos
  ...
  que o conjunto real.
- **A checagem de retorno de exceção é mais frouxa que a do SHERLOC, por causa
  do trace.** O SHERLOC compara o endereço de retorno exato; num trace por
  blocos, a interrupção cai no meio de um bloco e o monitor só sabe o bloco.
  Aceitamos então o *span* do bloco interrompido ou um sucessor dele no CFG —
  bounded pelo modelo, mas mais largo que a igualdade exata. Com ETM/MTB real,
  que reporta o endereço exato, a comparação estrita do SHERLOC é possível.
